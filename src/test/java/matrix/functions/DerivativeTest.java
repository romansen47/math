	...
	public static void setUpBeforeClass() throws Exception {
		...
		testfun=new Function() {
			...
			public IMatrix value(IMatrix input) throws Exception {
				return new Matrix(new double[][] {{Math.pow(input.getEntry(0, 0),2)
										+Math.pow(input.getEntry(0, 1),2)}});
			}
		};
		...
		IMinimizer min=new GradientMinimizer(1.e-5,1.e-5);
		
		IMatrix nul = min.find(testfun, new Matrix(new double[][]{{1,1}}));
