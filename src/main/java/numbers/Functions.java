...
public class Functions
{
	...
	public static Binary add2(Binary A, Binary B)
	{
		...
		boolean[] TMP1=new boolean[max+1];
		boolean[] TMP2=new boolean[max+1];
		...
		for (int i=0;i<max+1;i++)
		{
			...
			else
			{
				...
				{
					if ( (TMP1[i]==false && TMP2[i]==true) ||
							(TMP1[i]==true && TMP2[i]==false) )
					{
						ans[i]=false;
						tmp=true;
					}
	...
	{
		boolean[] ans=new boolean[tmp.length+1];
		for (int i=0;i<tmp.length;i++) { ans[i+1]=tmp.tuple[i]; }
		return(new Binary(ans));
	}
	...
	{
		boolean[] ans=new boolean[tmp.length-1];
		for (int i=0;i<ans.length;i++) { ans[i]=tmp.tuple[i+1]; }
		return(new Binary(ans));
	}
	...
	public static Binary mult2(Binary tmp1,Binary tmp2)
	{
		Binary A=tmp1.copy();
		Binary tmp;
		if (tmp2.tuple[0]==false){tmp=new Binary(0);}
		else {tmp=A.copy();}
		for (int i=1;i<tmp2.length;i++)
		{
			if (tmp2.tuple[i]==true) { tmp=add2(tmp,shiftright(A)); }
			A=shiftright(A);
		}
		return(tmp);
	}
	...
	{
		Binary n=m.copy();
		if (n.length==1) { return(new Binary(1)); }
		else
		{
			Binary fak=n.copy(); n.subone();
			while (n.length>1) { fak=mult2(fak,n); n.subone(); }
			return(fak);
		}
	}
	...
	{
		int LENGTH=Math.max(tmp1.length,tmp2.length)+1;
		int min=Math.min(tmp1.length,tmp2.length);
		int[] tuple=new int[LENGTH];
		{
			int rest=0;
			if (tmp1.length==tmp2.length)
			{
				...
				{
					int TMP=tmp1.tuple[LENGTH-2-i]+tmp2.tuple[LENGTH-i-2]+rest;
			...
			else
			{
				for (int i=0; i<min; i++) // LENGTH-1
       	                        {
               	                        int TMP=tmp1.tuple[tmp1.length-i-1]+tmp2.tuple[tmp2.length-i-1]+rest;
                       	                tuple[LENGTH-i-1]=TMP%Natural.MAX;
                               	        rest=TMP/Natural.MAX;
                               	}
				if (tmp1.length<tmp2.length)
				{
					for (int i=min; i<LENGTH-1; i++) // LENGTH-1
                        	        {
                                	        int TMP=tmp2.tuple[LENGTH-i-2]+rest;
                                        	tuple[LENGTH-i-1]=TMP%Natural.MAX;
                                        	rest=TMP/Natural.MAX;
                                	}
				}
				else
				{
					for (int i=min; i<LENGTH-1; i++) // LENGTH-1
                                	{
                                       		int TMP=tmp1.tuple[LENGTH-i-2]+rest;
                                        	tuple[LENGTH-i-1]=TMP%Natural.MAX;
                                        	rest=TMP/Natural.MAX;
                                	}
				}
		...
		Natural answer = new Natural(tuple);
		return(answer);
	}
	...
	public static Natural mult2(Natural tmp1,Natural tmp2)
	{
		...
		{
			Natural test=mult(tmp2.tuple[tmp2.length-1-i],A);
			tmp=add(tmp,test);
			A=shiftleft(A);
		}
		return(tmp);
